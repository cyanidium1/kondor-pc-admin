...
const configOption = {
  ...
  preview: {
    ...
    prepare({ label, priceDelta, isDefault }: Record<string, any>) {
      return {
        title: label,
        subtitle: priceDelta === 0 ? (isDefault ? "включено (за замовч.)" : "включено") : `+${priceDelta.toLocaleString("uk")} ₴`,
      };
    },
  },
};
...
export const build = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "number",
      group: "spec",
      description: "Множник для FPS відеокарти. 1.0 = без змін. 0.9 = -10% (слабший CPU). 1.05 = +5% (OC).",
      initialValue: 1.0,
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({ name: "brand", title: "Бренд", type: "string", validation: (R) => R.required() }),
            defineField({ name: "model", title: "Модель", type: "string", validation: (R) => R.required() }),
            defineField({ name: "displayName", title: "Повна назва (для UI)", type: "string", validation: (R) => R.required() }),
            defineField({
